...
func RunCommand(arg ...string) (stdOut *GeneratedFile) {
	return runCommand("", false, arg...)
}

func RunCommandSilent(arg ...string) (stdOut *GeneratedFile) {
	return runCommand("", true, arg...)
}
...
func RunCommandWithDir(dir string, arg ...string) (stdOut *GeneratedFile) {
	...
	return runCommand(dir, false, arg...)
}
...
func RunCommandWithDirSilent(dir string, arg ...string) (stdOut *GeneratedFile) {
	...
	return runCommand(dir, true, arg...)
}

func runCommand(dir string, silent bool, arg ...string) (stdOut *GeneratedFile) {
	...
	cmdKey := "exec command" // fast the key is clang target file path
	fast := true
	...
	s := setup{
		init: func() {
			binaryPath := arg[0]
			switch binaryPath {
			case "clang", "clang-format", "gcc", "g++", "cl", "link", "lib":
				// 编译链接工具执行速度快，使用快速模式
				cmdKey = filepath.Base(arg[len(arg)-1])
			case "ping", "go", "npm", "yarn", "pip", "cargo", "git", "svn":
				// 这些命令执行时间长或需要实时输出，使用慢速模式
				fast = false
			}
...
func ConvertUtf82Gbk(src string) string {
	if IsWindows() {
		// GB18030 编码转换，支持中文 Windows 控制台正确显示 UTF-8 文本
		c := mylog.Check2(simplifiedchinese.GB18030.NewDecoder().String(src))
