...
func TestFormatDuration(t *testing.T) {
	for _, one := range []struct {
		...
		assert.Equal(t, one.Expected, txt.FormatDuration(one.Duration, one.IncludeMillis))
	}
}
...
func TestParseDuration(t *testing.T) {
	for _, one := range []struct {
		...
		mylog.Call(func() {
			result := mylog.Check2(txt.ParseDuration(one.Input))
			// desc := fmt.Sprintf("Index %d: %s", i, one.Input)
			assert.Equal(t, one.ExpectedDuration, result)
		})
	}
}
