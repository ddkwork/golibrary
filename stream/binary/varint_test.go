...
func testVarint(t *testing.T, x int64) {
	...
	y = mylog.Check2(ReadVarint(bytes.NewReader(buf)))
	if x != y {
...
func testUvarint(t *testing.T, x uint64) {
	...
	y = mylog.Check2(ReadUvarint(bytes.NewReader(buf)))
	if x != y {
...
func TestBufferTooSmall(t *testing.T) {
	...
	for i := 0; i <= len(buf); i++ {
		...
		mylog.Call(func() { x = mylog.Check2(ReadUvarint(bytes.NewReader(buf))) })
	}
}
...
func TestOverflow(t *testing.T) {
	...
	testOverflow(t, []byte{0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF}, 1<<64-1, -11, overflow)
}
