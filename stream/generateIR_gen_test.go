...
func generateIR(path string, callBack func(b *stream.Buffer)) {
	...
	g.P("  }")
	g.P("  flag := os.O_WRONLY | os.O_CREATE | os.O_SYNC")
	g.P("  if isAppend {")
	g.P("   flag |= os.O_APPEND")
	g.P("  } else {")
	g.P("   flag |= os.O_TRUNC")
	g.P("  }")
	g.P("  f, e := os.OpenFile(name, flag, 0644)")
	g.P("  defer func() { mylog.Check(f.Close()) }()")
	g.P("  mylog.CheckIgnore(e)")
	g.P("  if e != nil {")
	g.P("   write(name, isAppend, data)")
	g.P("  }")
	g.P("  mylog.Check2(f.Write(NewBuffer(data).Bytes()))")
	g.P(" })")
	g.P("}")
	g.P("")
	...
	g.P("func (b *Buffer) Replace(old, new string, n int) *Buffer {")
	g.P(" s := strings.Replace(b.String(), old, new, n)")
	g.P(" b.Reset()")
	g.P(" b.WriteString(s)")
	g.P(" return b")
	g.P("}")
	...
	g.P(" runes := []rune(s)")
	g.P(" width := uniseg.StringWidth(string(runes))")
	g.P(" if width < length {")
	g.P("  repeat := strings.Repeat(\" \", length-width)")
	g.P("  ss = string(runes) + repeat")
	g.P("  return ss")
	g.P(" } else {")
	g.P("  if length <= len(runes) {")
	g.P("   return string(runes[:length])")
	g.P("  }")
	g.P("  return s")
	g.P(" }")
	g.P(" return s")
	...
	g.P("// CopyFS copies the file system fsys into the directory dir,")
	g.P("// creating dir if necessary.")
	g.P("//")
	g.P("// Newly created directories and files have their default modes")
	g.P("// where any bits from the file in fsys that are not part of the")
	g.P("// standard read, write, and execute permissions will be zeroed")
	g.P("// out, and standard read and write permissions are set for owner,")
	g.P("// group, and others while retaining any existing execute bits from")
	g.P("// the file in fsys.")
	g.P("//")
	g.P("// Symbolic links in fsys are not supported, a *PathError with Err set")
	g.P("// to ErrInvalid is returned on symlink.")
	g.P("//")
	g.P("// Copying stops at and returns the first error encountered.")
	g.P("func CopyFS(dir string, fsys fs.FS) error {")
	g.P(" return fs.WalkDir(fsys, \".\", func(path string, d fs.DirEntry, err error) error {")
	g.P("  if !filepath.IsLocal(path) {")
	g.P("   return err")
	g.P("  }")
	g.P("  newPath := filepath.Join(dir, path)")
	g.P("  if d.IsDir() {")
	g.P("   return os.MkdirAll(newPath, 0777)")
	g.P("  }")
	g.P("")

	g.P("  // TODO(panjf2000): handle symlinks with the help of fs.ReadLinkFS")
	g.P("  //   once https://go.dev/issue/49580 is done.")
	g.P("  //  we also need filepathlite.IsLocal from https://go.dev/cl/564295.")
	g.P("  if !d.Type().IsRegular() {")
	g.P("   return &os.PathError{Op: \"CopyFS\", Path: path, Err: os.ErrInvalid}")
	g.P("  }")
	g.P("  r := mylog.Check2(fsys.Open(path))")
	g.P("  defer func() { mylog.Check(r.Close()) }()")
	g.P("  info := mylog.Check2(r.Stat())")
	g.P("  w := mylog.Check2(os.OpenFile(newPath, os.O_CREATE|os.O_TRUNC|os.O_WRONLY, 0666|info.Mode()&0777))")
	g.P("  if _, e := io.Copy(w, r); e != nil {")
	g.P("   mylog.Check(w.Close())")
	g.P("   return &os.PathError{Op: \"Copy\", Path: newPath, Err: e}")
	g.P("  }")
	g.P("  return w.Close()")
	g.P(" })")
	g.P("}")
	...
	g.P("func CopyDir(dst, src string) {")
	g.P(" mylog.Check(CopyFS(dst, os.DirFS(src)))")
	g.P("}")
	g.P("")

	g.P("func copyFile(dst, src string) {")
