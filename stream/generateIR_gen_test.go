...
func generateIR(path string, callBack func(b *stream.Buffer)) {
	...
	g.P("  if _ := mylog.Check2(io.Copy(w, r)); err != nil {")
	g.P("   w.Close()")
	g.P("   return &os.PathError{Op: \"Copy\", Path: newPath, Err: err}")
	g.P("  }")
	g.P("  return w.Close()")
	g.P(" })")
	g.P("}")
	...
	g.P("  if entry.IsDir() {")
	g.P("   CopyDir(filepath.Join(dst, entry.Name()), filepath.Join(src, entry.Name()))")
	g.P("  } else {")
	g.P("   copyFile(filepath.Join(dst, entry.Name()), filepath.Join(src, entry.Name()))")
	g.P("  }")
	g.P(" }")
	g.P("}")
	g.P("")
